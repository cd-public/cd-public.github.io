...
int main() {
    uint32_t h_i[08];  /* 256 bit hash */
    uint32_t m_i[16];  /* 512 bit message chunk */
...
```
  ...
  - A 'size_t` 'size', how many things to read.
...
int main() {}
    ...
    fread(bytes, 1, 64, fp); /* read up to 512 bits */
    return 0;
}
```
...
*Endianness is an challenge topic. Be systematic, patient, and indefatigable.*
...
```
  ...
  2. Big-Endian
    ...
    - **Example**: The number `CDAB3412` is stored in memory as:
```email
CDAB3412
```
...
- They are responsible for the C89 eligible header `arpa/inet.h`
  - It contains e.g. `htonl`
    ...
    - In 1989, `uint32_t` was commonly called a long.
...
- In C89 compliance, we first declare variables.
...
```
- You will not this contains repeated code, and is therefore *de facto* wrong, though it runs correctly.
...
- The C89 "string.h" library contains a helpful function `memcpy`.
  - I have never once in my life remember destination is first, not source.
