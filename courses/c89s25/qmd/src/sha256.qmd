...
-   We note that 257 == 0x101 is prime.
    -   And therefore $\forall W : gcd(257,W) = 1$
```{python}
import math
...
x = 1
l = len(M.encode('utf-8')) * x.bit_length()
k = 512 - (l + 65) % 512
...
```{.C}
/* 32 bit bitwise complement, exact */
uint32_t and(uint32_t u, uint32_t v) {
    return u & v;
}
...
- C ddition is a binary bitwise operation:
...
\sigma_0(U)&= \textsf{ROTR}^{07}(U) \oplus \textsf{ROTR}^{18}(U) \oplus \textsf{SHR}^{3}(U) \\
...
\begin{align}
\begin{split}
& T_1  = H + \Sigma_1(E) + \textsf{Ch}(E,F,G) + K_j + W_j \\
& T_2  = \Sigma_0(A) + \textsf{Maj}(A,B,C) \\
\end{split}
\end{align}
$$
...
- Concludes the work on $M^{(i)}$ by finding $H^{(i)}$
