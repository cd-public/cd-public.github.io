...
```{r}
ggplot(wine, aes(x = points, y = price, color = bordeaux)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Multiple Regression of Price on Points and Bordeaux",
       x = "Points",
       y = "Price")
```
...
ggplot(wine, aes(x = points, y = price, color = bordeaux)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Interaction Model: Price on Points and Bordeaux",
       x = "Points",
       y = "Price")
```
...
5. **Bonus+:** Include regression lines for Bordeaux vs. not
6. **Bonus++:**Pick novel binary variable from "designation" column...
...
m2 <- lm(price ~ points+Reserve, data = extra)
get_regression_table(m2)
```
## Extra 2
```{r}
extra = wine %>%
  mutate("Reserve"=grepl("Reserve", designation, ignore.case = TRUE) | 
...
extra = wine %>%
  mutate("Reserve"=grepl("Reserve", designation, ignore.case = TRUE) | 
           grepl("Reserva", designation, ignore.case = TRUE) | 
           grepl("Riserva", designation, ignore.case = TRUE))
m2 <- lm(price ~ points+Reserve, data = extra)
get_regression_table(m2)
```
...
- There are 18 transactions with risk above 77. 12 are actually fraud. 20 fraudulent transactions have risk below 77.
- What are precision, recall, and accuracy?
- Precision: 12 / (12 + 6) = 67%
- Recall: 12 / (12 + 20) = 37.5%
- Accuracy: (12 + 962)/1000 = 97.4%
