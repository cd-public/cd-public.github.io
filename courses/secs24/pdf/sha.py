...
def right_rotate(x, n):
    """Perform a right rotation on x by n bits."""
    return (x >> n) | (x << (32 - n)) & 0xFFFFFFFF

def majority(x, y, z):
    """Calculate the majority of x, y, and z."""
    return (x & y) ^ (x & z) ^ (y & z)

def choose(x, y, z):
    """Choose x if x is True, otherwise choose z."""
    return (x & y) ^ (~x & z)
...
def sha256(message):
    ...
    hash_hex = ''.join(format(h, '08x') for h in H)
    return hash_hex

# Example usage:
message = b"Hello, World!"
hashed_message = sha256(message)
print("SHA-256 hash of", message.decode(), ":", hashed_message)

import hashlib

def sha256_hl(message):
...
# Example usage:
message = b"Hello, World!"
hashed_message = sha256_hl(message)
print("SHA-256 hash of", message.decode(), ":", hashed_message)